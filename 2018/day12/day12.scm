...
;  right, and N is whether the current pot will have a plant in the next generation.
;  For example:
;
...
(define (get-info filename)
  (let ((info (make-hash))
    ...
    (for ((line lines))
         (when (regexp-match #rx"initial state" line)
           (define init (car (regexp-match* #rx"([\\.\\#]+)" line)))
           (hash-set! info 'initial init))
         (when (regexp-match #rx" => " line)
           (define-values (rule result) (apply values (car (regexp-match* #rx"([\\.\\#]+) => ([\\.\\#])" line #:match-select cdr))))
           (hash-set! info rule result)))
    info))

(define (generation info states)
  (let ((input-str (string-append "..." states "..."))
        (output-str ""))
    (for ((i (range (- (string-length input-str) 4))))
         (let* ((key-str (substring input-str i (+ i 5)))
                (current (substring key-str 2 3)))
           (if (hash-has-key? info key-str)
               (set! output-str (string-append output-str (hash-ref info key-str)))
               (set! output-str (string-append output-str ".")))))
    output-str))

(define (tot-gen gen-str gen-cnt)
  (let ((tot 0)
        (offset (* -1 (+ gen-cnt 2))))
    (for ((c (string->list gen-str)))
         (when (char=? c #\#)
           (set! tot (+ tot offset)))
         (set! offset (add1 offset)))
    tot))

(define (part-one filename (gen-cnt 20))
  (let* ((info (get-info filename))
         (gen-str (string-append ".." (hash-ref info 'initial) ".."))
         (tot 0) (prev-tot 0) (prev-diff 0))
    (for ((g (range 1 (add1 gen-cnt))))
         (set! gen-str (generation info gen-str))
         (set! tot (tot-gen gen-str g))
;         (printf "~a: ~a ~a~n" g tot (- tot prev-tot))
         (set! prev-diff (- tot prev-tot))
         (set! prev-tot tot))
    (cons (tot-gen gen-str gen-cnt) prev-diff)))

(check-equal? (car (part-one "test-data.txt")) 325)
(check-equal? (car (part-one "day12-data.txt")) 2995)
...
;  numbers of all pots which contain a plant? (Answer: 3650000000377)
;

(define (part-two filename)
  (let* ((gen-cnt 2000)
         (results (part-one filename gen-cnt))
         (tot (car results))
         (diff (cdr results)))
    (+ tot (* (- 50000000000 gen-cnt) diff))))

(check-equal? (part-two "test-data.txt") 999999999374)
(check-equal? (part-two "day12-data.txt") 3650000000377)
