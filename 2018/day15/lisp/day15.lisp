...
(defstruct (unit (:constructor make-unit) (:conc-name unit-))
  type alive attack hps)

(defparameter *new-goblin* (make-unit :type 'goblin :alive t :attack 3 :hps 200))
(defparameter *new-elf*    (make-unit :type 'elf    :alive t :attack 3 :hps 200))

(defun reading-order (p1 p2)
  (cond
    ((< (cdr p1) (cdr p2)) t)
    ((> (cdr p1) (cdr p2)) nil)
    (t (< (car p1) (car p2)))))
    ...
    (with-open-file (input filename :direction :input)
      (loop for ch = (read-char input nil nil) 
            while ch do
              (case ch
                ((#\# #\.)
                 (setf (gethash (cons x y) cave) ch)
                 (incf x))
                (#\G
                 (setf (gethash (cons x y) units) *new-goblin*)
                 (setf (gethash (cons x y) cave) #\.)
                 (incf x))
                (#\E
                 (setf (gethash (cons x y) units) *new-elf*)
                 (setf (gethash (cons x y) cave) #\.)
                 (incf x))
                (#\newline
                 (incf y)
...
(defun draw-cave (cave units)
  (let* ((keys (loop for key being the hash-keys of cave collect key))
         (max-x (apply #'max (mapcar #'car keys)))
         (max-y (apply #'max (mapcar #'cdr keys))))
    (dotimes (y (1+ max-y))
      (dotimes (x (1+ max-x))
        (if (gethash (cons x y) units)
            (if (eq (unit-type (gethash (cons x y) units)) 'goblin)
                (format t "G")
                (format t "E"))
            (format t "~a" (gethash (cons x y) cave))))
      (format t "~%"))
    (format t "~%")))

(defun get-units (units type)
  (let ((return-units '()))
    (maphash (lambda (coor unit)
               (when (equal (unit-type unit) type)
                 (push unit return-units)))
             units)
    return-units))

(defun part-one (filename)
  (multiple-value-bind (cave units) (load-cave filename)
    (labels ((loop (rnd)
               (maphash (lambda (unit-coor unit)
                          (let ((enemy (if (eq (unit-type unit) 'elf) 'goblin 'elf)))
                            ;; Combat?
                            (when (> (length (get-units units enemy)) 0)
                              ;; Placeholder for combat logic
                              )))
                        units)
               ;; Placeholder for condition to continue the loop
               ;; (when (not (zero? (length (get-units units 'goblin))))
               ;;   (loop (1+ rnd)))
               ))
      (loop 0))))
