...
;  of the changes in frequency have been applied?  (Answer: 427)
;

(define (part-one filename)
  (apply + (map string->number (regexp-match* #rx"([0-9+-]+)" (file->string filename)))))

(check-equal? (part-one "test-data.txt") 3)
(check-equal? (part-one "day01-data.txt") 427)
...
;
; What is the first frequency your device reaches twice? (Answer: 341)
;

(define (part-two filename)
  (let* ((raw-str (file->string filename))
         (numbers (map string->number (regexp-match* #rx"([0-9+-]+)" raw-str)))
         (history (make-hash))
         (freq 0))
    (let loop ((idx 0))
         (set! freq (+ freq (list-ref numbers idx)))
         (if (hash-has-key? history freq)
             (error (format "frequency hit a second time: ~a" freq))
             (hash-set! history freq #t))
         (loop (modulo (add1 idx) (length numbers))))))
