...
;  it contains all other nodes in the tree (or contains nodes that contain nodes, and
;  so on).
;
...
;
; What is the sum of all metadata entries? (Answer: 44893)
;

; And sometimes you have to restart a third time! :-D

(define total 0)

(define (traverse number-stream)
  (let ((nodes (read number-stream))
        (meta-cnt (read number-stream)))
    (for ((i (range nodes)))
         (traverse number-stream))
    (for ((i (range meta-cnt)))
         (set! total (+ (read number-stream) total)))))

(define (part-one filename)
  (set! total 0)
  (traverse (open-input-string (file->string filename)))
  total)

(check-equal? (part-one "test-data.txt") 138)
(check-equal? (part-one "day08-data.txt") 44893)
...
;
; What is the value of the root node? (Answer: 27433)
;


(define (collect-nodes node number-stream)
  (let ((nodes (read number-stream))
        (meta-cnt (read number-stream)))
    (cond
      ((zero? nodes)
       (hash-set! node 'type 'leaf)

       (hash-set! node 'total 0)
       (for ((i (range meta-cnt)))
            (hash-set! node 'total (+ (read number-stream) (hash-ref node 'total)))))

      (else
        (hash-set! node 'type 'node)

        (hash-set! node 'children (make-hash))
        (for ((i (range nodes)))
             (hash-set! (hash-ref node 'children) (add1 i) (collect-nodes (make-hash) number-stream)))

        (hash-set! node 'meta '())
        (for ((i (range meta-cnt)))
             (hash-set! node 'meta (cons (read number-stream) (hash-ref node 'meta))))
        
        (hash-set! node 'total 0)
        (for ((i (hash-ref node 'meta)))
             (when (hash-has-key? (hash-ref node 'children) i)
               (hash-set! node 'total (+
                                        (hash-ref node 'total)
                                        (hash-ref (hash-ref (hash-ref node 'children) i) 'total)))))
        
        )))
  node)

(define (part-two filename)
  (let* ((number-stream (open-input-string (file->string filename)))
         (root (collect-nodes (make-hash) number-stream)))
    (hash-ref root 'total)))

(check-equal? (part-two "test-data.txt") 66)
(check-equal? (part-two "day08-data.txt") 27433)
