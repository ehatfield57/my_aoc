...
(require rackunit)
(require data/double-linked-ring) ; https://github.com/matteo-daddio/double-linked-ring
...
;
; What is the winning Elf's score? (Answer: 367802)
;

(define (part-one players points)
  (let ((player-scores (make-hash))
        (marbles (ring '(0))))

   (for ((p (range players)))
        (hash-set! player-scores (add1 p) 0))
   
  (for ((i (range (add1 points))))
       (cond
         ((zero? (modulo (add1 i) 23))
          (define player (add1 (modulo i players)))
          (hash-set! player-scores player (+ (hash-ref player-scores player) (add1 i)))

          (for ((x (range 7))) (ring-prev! marbles))
          (hash-set! player-scores player (+ (hash-ref player-scores player) (ring-current marbles)))
          (ring-remove-current! marbles))

         (else
           (ring-next! marbles)
           (ring-add-next! marbles (add1 i)))
         ))
  (apply max (hash-values player-scores))))
...
(check-equal? (part-one 493 71863) 367802)
...
;  marble were 100 times larger? (Answer: 2996043280)
;

(check-equal? (part-one 493 7186300) 2996043280)
