...
(define (part-one cycles)
  (let* ((recipe "37")
         (elf1 0)
         (elf2 1)
         (get-num (lambda (idx) (string->number (substring recipe idx (add1 idx)))))
         (new-elf (lambda (elf) (modulo (+ 1 elf (get-num elf)) (string-length recipe))))
         (new-sum (lambda () (number->string (+ (get-num elf1) (get-num elf2))))))
    (let loop ((t 0))
     (set! recipe (string-append recipe (new-sum)))
     (set! elf1 (new-elf elf1))
     (set! elf2 (new-elf elf2))
     (when (< t (+ 10 cycles)) (loop (add1 t))))
    (substring recipe cycles (+ 10 cycles))))

(check-equal? (part-one    9) "5158916779")
(check-equal? (part-one    5) "0124515891")
(check-equal? (part-one   18) "9251071085")
(check-equal? (part-one 2018) "5941429882")

;(check-equal? (part-one 607331) "8610321414") ; :-D  Worked! (but took awhile)
...
(define (part-two goal)
  (let* ((recipe (make-vector 22000000))
         (elf1 0)
         (elf2 1)
         (end-idx 2)
         (get-num (lambda (idx) (vector-ref recipe idx)))
         (new-elf (lambda (elf) (modulo (+ 1 elf (get-num elf)) end-idx)))
         (add-num (lambda (num) (vector-set! recipe end-idx num) (add1 end-idx)))
         (sub-vec (lambda (vec) (apply string-append (map number->string (vector->list vec)))))
         (show-recipe (lambda () (sub-vec (vector-copy recipe 0 end-idx)))))
    (vector-set! recipe 0 3)
    (vector-set! recipe 1 7)
    (let loop ((t 0))
     (when (zero? (modulo t 1000)) (printf "~a~n" end-idx))
     (define new-num (+ (get-num elf1) (get-num elf2)))
     (cond
       ((> new-num 9)
        (set! end-idx (add-num 1))
        (set! end-idx (add-num (remainder new-num 10))))
       (else
         (set! end-idx (add-num new-num))))

     (set! elf1 (new-elf elf1))
     (set! elf2 (new-elf elf2))

     (define end-str (sub-vec (vector-copy recipe (max 0 (- end-idx 7)) (+ end-idx 7))))
     (if (string-contains? end-str goal)
         (- end-idx (string-length goal))
         (loop (add1 t))))))

(check-equal? (part-two "51589")    9)
(check-equal? (part-two "01245")    5)
(check-equal? (part-two "92510")   18)
(check-equal? (part-two "59414") 2018)

(check-equal? (part-two "607331") 20258123) ; SOOOOOO Close, my answer is 20258124
