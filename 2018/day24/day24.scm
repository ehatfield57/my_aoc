...
;  group's attack type and the defending group's immunities and weaknesses. By
;  default, an attacking group would deal damage equal to its effective power to the
;  defending group. However, if the defending group is immune to the attacking group's
;  attack type, the defending group instead takes no damage; if the defending group
;  is weak to the attacking group's attack type, the defending group instead takes
;  double damage.
;
; The defending group only loses whole units from damage; damage is always dealt in
;  such a way that it kills the most units possible, and any remaining damage to a
;  unit that does not immediately kill it is ignored. For example, if a defending
...
;  have? (Answer: 25524) (Answer from Reddit Python code)
;

; Used Reddit Python code to solve this one
...
;  smallest boost it needs to win? (Answer: 4837)
;

; Didn't even try, solved by Reddit rust example
