...
;
; What is the number of the Sue that got you the gift? (Answer: 103)
;

(define truth #hash(("children" . 3)
                    ("cats" . 7)
                    ("samoyeds" . 2)
                    ("pomeranians" . 3)
                    ("akitas" . 0)
...
(define (part-one item amt)
  (not (= (hash-ref truth item) amt)))

(define (find-aunt compare-truth)
  (let ((aunts (make-vector 500 #t))
        (results '()))
    (call-with-input-file
      ...
      (lambda (in-port)
        (do ((line (read-line in-port 'any) (read-line in-port 'any)))
            ((eof-object? line))
            (let ((sue-num (string->number (cadr (regexp-match #rx"^Sue ([0-9]+):" line))))
                   (groups (regexp-match* #rx"(([a-z]+): ([0-9]+))+" line)))
              (for ((group groups))
                   (let* ((pieces (cdr (regexp-match #rx"^([a-z]+): ([0-9]+)" group)))
                           (item (car pieces))
                           (amt (string->number (cadr pieces))))
                     (when (compare-truth item amt)
                       (vector-set! aunts (sub1 sue-num) #f))))))))
    (for ((i 500))
         (when (eqv? #t (vector-ref aunts i))
           (set! results (cons (add1 i) results))))
    results))

(printf "Part One: Aunt ~a~n" (find-aunt part-one))
...
;  magnetoreluctance).
;
; What is the number of the real Aunt Sue? (Answer: 405)
;

(define (part-two item amt)
  (cond
    ((or (string=? item "cats") (string=? item "trees"))
     (> (hash-ref truth item) amt))
    ((or (string=? item "pomeranians") (string=? item "goldfish"))
     (< (hash-ref truth item) amt))
    (else
      (not (= (hash-ref truth item) amt)))))

(printf "Part Two: Aunt ~a~n" (find-aunt part-two)) ; Don't know why we get two answers
