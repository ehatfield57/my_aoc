...
;  the strings in total for the entire file? (Answer: 1371)
;
...
(define (part-one filename)
  ...
  (let ((before-sum 0)
    ...
    (with-input-from-file
      ...
      (lambda ()
        (for ([line (in-lines)])
          (let* ((before-num (string-length line))
                 (read-str (format (read in-port)))
                 (after-num  (string-length read-str)))
            (printf "Line ~a (~a) -> '~a' (~a)~n" line before-num read-str after-num)
            (set! before-sum (+ before-sum before-num))
            (set! after-sum  (+ after-sum after-num))))))

    (printf "Total original length: ~a, after: ~a, subtract: ~a~n" before-sum after-sum (- before-sum after-sum)))
  (close-input-port in-port))

; (part-one "day8-data.txt")
(part-one "foo.txt")
...
;  part of this puzzle) is 42 - 23 = 19. (Answer: 2117)
; 
...
(define (encoded line)
  (define (foo lst)
    (cond
      ((null? lst) '())
      ((char=? #\\ (car lst)) (cons #\\ (cons (car lst) (foo (cdr lst)))))
      ((char=? #\" (car lst)) (cons #\\ (cons (car lst) (foo (cdr lst)))))
      (else (cons (car lst) (foo (cdr lst))))))
  (foo (string->list line)))
  ...
  (let ((before-sum 0)
    ...
    (with-input-from-file
      ...
      (lambda ()
        (for ([line (in-lines)])
          (let* ((before-num (string-length line))
                (encoded-lst (encoded line))
                (after-num (length encoded-lst)))
           (printf "Line ~a (~a) -> ~a (~a)~n" line before-num encoded-lst (+ 2 after-num))
           (set! before-sum (+ before-sum before-num))
           (set! after-sum (+ 2 after-sum after-num))))))
    (printf "Total original length: ~a, after: ~a, subtract: ~a~n" before-sum after-sum (- after-sum before-sum)))
  (close-input-port in-port))

; (part-two "foo.txt")
(part-two "day8-data.txt")
