...
(define test-data (format "0\t2\t7\t0"))

(define (redistribute data)
  (let* ((max-num (apply max (vector->list data)))
         (max-idx (vector-member max-num data))
         (fit-idx (lambda (n) (modulo n (vector-length data)))))
    (vector-set! data max-idx 0)
    (for ((idx (range (add1 max-idx) (+ 1 max-idx max-num))))
         (vector-set! data (fit-idx idx) (add1 (vector-ref data (fit-idx idx)))))
    data))

(define (create-key data)
  (string-join (map number->string (vector->list data)) "-"))

(define (part-one data-str)
  (let* ((data (list->vector (map string->number (string-split data-str "\t"))))
         (history (make-hash))
         (return 0))
    (let loop ((cycle 0))
     (printf "Hi Edward A, cycle: ~a, data: ~a~n" cycle data)
     (set! return cycle)
     (let ((data-key (create-key data)))
      (when (not (hash-has-key? history data-key))
        (hash-set! history data-key #t)
        (redistribute data)
        (loop (add1 cycle)))))
    return))

(check-equal? (part-one test-data) 5)

(check-equal? (part-one (car (file->lines "day06-data.txt"))) 7864)
...
;  configuration in your puzzle input? (Answer: 1695)
;

(define prev-end "10\t9\t8\t7\t6\t5\t4\t3\t1\t1\t0\t15\t14\t13\t11\t12")

(check-equal? (part-one prev-end) 1695)
