...
;
; How many steps does it take to reach the exit? (Answer: 360603)
;

(define test-data (vector 0 3 0 1 -3))

(define (steps-to-exit jumps-orig)
  (let* ((jumps (vector-copy jumps-orig))
         (jump-len (vector-length jumps))
         (steps 0))
    (let loop ((p-index 0))
     (define cur-jmp (vector-ref jumps p-index))
     (vector-set! jumps p-index (add1 cur-jmp))
     (set! steps (add1 steps))
     (define next-p-index (+ p-index cur-jmp))
     (when (not (or (< next-p-index 0) (>= next-p-index jump-len)))
       (loop next-p-index)))
    steps))

(check-equal? (steps-to-exit test-data) 5)

(define (part-one filename)
  (let* ((lines (file->lines filename))
         (jumps (list->vector (map string->number lines))))
    (steps-to-exit jumps)))

(check-equal? (part-one "day05-data.txt") 360603)
...
;
; How many steps does it now take to reach the exit? (Answer: 25347697)
;

(define (weird-steps jumps)
  (let ((jump-len (vector-length jumps))
        (steps 0))
    (let loop ((p-index 0))
     (define cur-jmp (vector-ref jumps p-index))

     (if (>= cur-jmp 3)
         (vector-set! jumps p-index (sub1 cur-jmp))
         (vector-set! jumps p-index (add1 cur-jmp)))
...
(check-equal? (weird-steps test-data) 10)
...
(check-equal? (part-two "day05-data.txt") 25347697)
