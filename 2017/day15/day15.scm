...
;  the remainder of dividing that resulting product by 2147483647. That final
...
;
; After 40 million pairs, what is the judge's final count? (Answer: 600)
;

; Function 'pad-left' borrowed from Day 14:
(define (pad-left width ch str)
  (if (>= (string-length str) width)
      str 
      (pad-left width ch (string-append ch str))))

(define (generator factor start-num)
  (let ((cur-num start-num))
   (lambda (cmd . args)
     (case cmd
       ((show) cur-num)
       ((next)
        (let ((new-num (remainder (* cur-num factor) 2147483647)))
         (set! cur-num new-num)
         new-num))
       ((bits)
        (let ((bit-num (pad-left 16 "0" (number->string cur-num 2))))
         (substring bit-num (- (string-length bit-num) 16))))))))

(define (part-one gen-a-num gen-b-num)
  (let ((gen-a (generator 16807 gen-a-num))
        (gen-b (generator 48271 gen-b-num))
        (total 0))
    (for ((i (range 40000000)))
         (let ((gen-a-num (gen-a 'next))
               (gen-a-16b (gen-a 'bits))
               (gen-b-num (gen-b 'next))
               (gen-b-16b (gen-b 'bits)))
           (when (string=? gen-a-16b gen-b-16b)
             (set! total (add1 total)))))
    total))

;(check-equal? (part-one 65 8921) 588)
;(check-equal? (part-one 699 124) 600)
...
;  It keeps track of the order it receives values; the first values from
;  each generator are compared, then the second values from each generator,
...
;  judge's final count? (Answer: 313)
;

(define (gen2 factor start-num multiple)
  (let ((cur-num start-num))
   (lambda (cmd . args)
     (case cmd
       ((show) cur-num)
       ((next)
        (let loop ((current cur-num))
         (define new-num (remainder (* current factor) 2147483647))
         (set! cur-num new-num)
         (if (not (zero? (modulo new-num multiple)))
             (loop new-num)
             new-num)))
       ((bits)
        (let ((bit-num (pad-left 16 "0" (number->string cur-num 2))))
...
(define (part-two gen-a-num gen-b-num)
  (let ((gen-a (gen2 16807 gen-a-num 4))
        (gen-b (gen2 48271 gen-b-num 8))
        (total 0))
    (for ((i (range 5000000)))
         (let ((gen-a-num (gen-a 'next))
               (gen-a-16b (gen-a 'bits))
...
(check-equal? (part-two 65 8921) 309)
(check-equal? (part-two 699 124) 313)
