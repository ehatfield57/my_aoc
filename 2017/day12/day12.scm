...
;
; How many programs are in the group that contains program ID 0? (Answer: 141)
;

(define (traverse graph visited node)
  (when (not (hash-has-key? visited node))
    (hash-set! visited node (hash-ref graph node))
    (for ((child (hash-ref graph node)))
         (traverse graph visited child))))

(define (get-graph filename)
  (let ((lines (file->lines filename))
        (graph (make-hash)))
    (for ((line lines))
         (define numbers (flatten (regexp-match* #rx"([0-9]+)" line #:match-select cdr)))
         (hash-set! graph (car numbers) (rest numbers)))
    graph))

(define (part-one filename)
  (let ((graph (get-graph filename))
        (visited (make-hash)))
    (traverse graph visited "0")
    (length (hash-keys visited))))

(check-equal? (part-one "test-data.txt") 6)
(check-equal? (part-one "day12-data.txt") 141)
...
;
; How many groups are there in total? (Answer: 171)
;
...
(define (part-two filename)
  (let ((graph (get-graph filename))
   ...
   (for ((node (hash-keys graph)))
        (when (not (hash-has-key? visited node))
          (traverse graph visited node)
          (set! group-cnt (add1 group-cnt))))
   group-cnt))

(check-equal? (part-two "test-data.txt") 2)
(check-equal? (part-two "day12-data.txt") 171)
