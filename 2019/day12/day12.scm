...
;  of a big distracting red spot, extreme radiation, and a whole lot of
;  moons swirling around. You decide to start by tracking the four largest
...
;  all moons' velocities have been updated, update the position of every
;  moon by applying velocity. Time progresses by one step once all of the
...
;  has a x position of 5, then Ganymede's x velocity changes by +1 (because
;  5 > 3) and Callisto's x velocity changes by -1 (because 3 < 5). However,
;  if the positions on a given axis are the same, the velocity on that axis
...
;  values of its x, y, and z position coordinates. A moon's kinetic energy
;  is the sum of the absolute values of its velocity coordinates. Below,
;  each line shows the calculations for a moon's potential energy (pot),
...
;  given in your scan for 1000 steps? (Answer: 7988)
;
...
;  a previous state? (Answer: 337721412394184)
;
