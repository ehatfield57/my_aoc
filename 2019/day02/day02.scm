...
;  represent the first opcode (1, addition), the positions of the two inputs (9 and
;  10), and the position of the output (3). To handle this opcode, you first need to
;  get the values at the input positions: position 9 contains 30, and position 10
;  contains 40. Add these numbers together to get 70. Then, store this value at the
;  output position; here, the output position (3) is at position 3, so it overwrites
...
;  is left at position 0 after the program halts? (Answer: 2692315)
;

(define (load-program filename)
  (let ((numbers (file->string filename)))
   (list->vector (map string->number (string-split (string-trim numbers) ",")))))

(define (execute program)
  (let ((get-program-position (lambda (pos) (vector-ref program (vector-ref program pos))))
        (set-program-position (lambda (pos val) (vector-set! program (vector-ref program pos) val))))
   (let loop ((program-idx 0))
    (case (vector-ref program program-idx)
      ((99)
       program)
      ((1)
       (set-program-position (+ program-idx 3) (+ (get-program-position (+ program-idx 1))
                                                 (get-program-position (+ program-idx 2))))
       (loop (+ program-idx 4)))
      ((2)
       (set-program-position (+ program-idx 3) (* (get-program-position (+ program-idx 1))
                                                 (get-program-position (+ program-idx 2))))
...
(define (part-one filename)
  (let ((program (load-program filename)))
   (when (string=? filename "day02-data.txt")
     (vector-set! program 1 12)
     (vector-set! program 2  2))
   (vector-ref (execute program) 0)))

(check-equal? (part-one "test1-data.txt") 3500)
(check-equal? (part-one "test2-data.txt") 2)
(check-equal? (part-one "test3-data.txt") 2)
(check-equal? (part-one "test4-data.txt") 2)
(check-equal? (part-one "test5-data.txt") 30)

(check-equal? (part-one "day02-data.txt") 2692315)
...
;  the computer, this is always 4 (1 opcode + 3 parameters) for the add and multiply
...
(define (part-two filename)
  ...
  (for ((noun (range 100)))
       (for ((verb (range 100)))
            (define test-program (vector-copy program))
            (vector-set! test-program 1 noun)
            (vector-set! test-program 2 verb)
            (when (= goal (vector-ref (execute test-program) 0))
              (printf "Noun:~a and Verb:~a produce ~a~n" noun verb goal)
              (printf "Part-two solution is:~a~n" (+ verb (* 100 noun))))))))

(part-two "day02-data.txt")
