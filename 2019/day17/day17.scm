...
;  Unfortunately, this has cut off the Wi-Fi for many small
;  robots that, unaware of the impending danger, are now trapped
;  on exterior scaffolding on the unsafe side of the shield.
;  To rescue them, you'll have to act quickly!
;
; The only tools at your disposal are some wired cameras and a
;  small vacuum robot currently asleep at its charging station.
...
;  cameras and the vacuum robot. Currently, because the vacuum
;  robot is asleep, you can only access the cameras.
;
; Running the ASCII program on your Intcode computer will provide
;  the current view of the scaffolds. This is output, purely coincidentally,
...
;  the scaffold intersections? (Answer: 7280)
;

; Used ChatGPT-o1-mini and Reddit Rust contestant answer to solve this.

;
; --- Part Two ---
;
; Now for the tricky part: notifying all the other robots about the solar flare. The
;  vacuum robot can do this automatically if it gets into range of a robot. However,
;  you can't see the other robots on the camera, so you need to be thorough instead:
...
;  use input instructions to receive them, but they need to be provided as ASCII code;
;  end each line of logic with a single newline, ASCII code 10.
;
; First, you will be prompted for the main movement routine. The main routine may only
;  call the movement functions: A, B, or C. Supply the movement functions to use as
;  ASCII text, separating them with commas (,, ASCII code 44), and ending the list with
;  a newline (ASCII code 10). For example, to call A twice, then alternate between B and
;  C three times, provide the string A,A,B,C,B,C,B,C and then a newline.
;
...
;  functions may not call other movement functions. Again, separate the actions with
;  commas and end the list with a newline. For example, to move forward 10 units, turn
;  left, move forward 8 units, turn right, and finally move forward 6 units, provide the
;  string 10,L,8,R,6 and then a newline.
;
; Finally, you will be asked whether you want to see a continuous video feed; provide
;  either y or n and a newline. Enabling the continuous video feed can help you see what's
;  going on, but it also requires a significant amount of processing power, and may even
;  cause your Intcode computer to overheat.
;
; Due to the limited amount of memory in the vacuum robot, the ASCII definitions of the
;  main routine and the movement functions may each contain at most 20 characters, not
...
; ....#####......
;
; In order for the vacuum robot to visit every part of the scaffold at least once, one
;  path it could take is:
;
; R,8,R,8,R,4,R,4,R,8,L,6,L,2,R,4,R,4,R,8,R,8,R,8,L,6,L,2
;
; Without the memory limit, you could just supply this whole string to function A and
;  have the main routine call A once. However, you'll need to split it into smaller parts.
;
; One approach is:
;
...
;  flare, it also can't help but leave them squeaky clean, collecting any space dust
;  it finds. Once it finishes the programmed set of movements, assuming it hasn't
;  drifted off into space, the cleaning robot will return to its docking station and
...
;  vacuum robot report it has collected?  (Answer: 1045393)
;
