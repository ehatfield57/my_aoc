defmodule Intcode do
  ...
  defp execute(intcode, input) do
    opcode = get_memory(intcode, intcode.pointer)
    {instruction, params} = parse_instruction(opcode)

    case instruction do
      :add ->
        param1 = get_param(intcode, intcode.pointer + 1, params[0])
        param2 = get_param(intcode, intcode.pointer + 2, params[1])
        dest = get_address(intcode, intcode.pointer + 3, params[2])
      ...
      :input ->
        case input do
          [first | rest] ->
            dest = get_address(intcode, intcode.pointer + 1, params[0])
      ...
      :output ->
        param = get_param(intcode, intcode.pointer + 1, params[0])
...
defmodule ScaffoldRobot do
  def main do
    ...
    # These should be determined manually based on the scaffold map from Part 1
    # Example movement routines (these need to be replaced with the actual solution)
    ...
    dust_amount = final_intcode.outputs |> Enum.reverse() |> hd()
