...
;  has value 3. The result of this operation, 33 * 3 = 99, is written according
...
;  what diagnostic code does the program produce? (Answer: 13285749)
;

(define (part-one filename (input-value 0))
  (let* ((intcode-str (file->string filename))
         (ic-interp (make-intcode-interpreter intcode-str)))
    (ic-interp 'push-io input-value)
    (ic-interp 'execute)
    (car (ic-interp 'get-io))))

(check-equal? (part-one "day05-data.txt" 1) 13285749)
...
;    it stores 1 in the position given by the third parameter. Otherwise,
;    it stores 0.
;
...
;  the ID of the system to test, provide it 5, the ID for the ship's thermal radiator
;  controller. This diagnostic test suite only outputs one number, the diagnostic code.
;
; What is the diagnostic code for system ID 5? (Answer: 5000972) (Reddit Python solution)
;
