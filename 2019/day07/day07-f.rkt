...
(define (ensure-memory-size intcode index)
  (if (>= (length intcode) (+ index 1))
      intcode
      (append intcode (make-list (- (+ index 1) (length intcode)) 0))))
...
(define (run-intcode intcode inputs)
  (define (step pc input outputs)
    (let* ([instruction (list-ref intcode pc)]
           ...
           [intcode (ensure-memory-size intcode param1)]
           [intcode (ensure-memory-size intcode param2)]
           [intcode (ensure-memory-size intcode param3)]
...
(define (amplifier-sequence intcode phases)
  (foldl (lambda (phase input)
           (let ([result (run-intcode intcode (list phase input))])
             (if (and (pair? result) (not (null? result)))
                 (car result)
                 0)))  ; Default to 0 if no output is produced
         0
         phases))
...
(define input (file->string "day07-data.txt"))
