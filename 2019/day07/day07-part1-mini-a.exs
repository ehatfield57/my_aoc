defmodule AmplificationCircuit do
  ...
  defp run_intcode(program, inputs, pointer) do
    ...
    case instruction do
      ...
      4 -> # Output
        val = get_param_value(program, pointer + 1, Enum.at(modes, 0))
        # Return the output signal
        val

      5 -> # Jump-if-true
        val = get_param_value(program, pointer + 1, Enum.at(modes, 0))
        target = get_param_value(program, pointer + 2, Enum.at(modes, 1))
        if val != 0 do
  ...
  defp get_params(program, pointer, modes, num_params) do
    ...
    # The destination is always in position mode
    dest = Enum.at(program, pointer + num_params + 1, 0)
    case num_params do
      2 -> {Enum.at(vals, 0), Enum.at(vals, 1), Enum.at(program, pointer + 3)}
