defmodule AdventOfCode2019.Day7 do
  ...
  def amplifier_loop_sequence(intcode, phases) do
    # Create independent states for each amplifier by copying the initial intcode
    states = Enum.map(phases, fn phase ->
      {Enum.map(intcode, & &1), [phase], 0}  # Create a fresh copy of intcode for each amplifier
    end)
