defmodule AdventOfCode2019.Day7 do
  ...
  def run_intcode(intcode, inputs, pc \\ 0) do
    ...
    IO.inspect(%{
      pc: pc,
      opcode: opcode,
      intcode_slice: Enum.slice(intcode, pc, 10)  # Print a portion of the intcode for debugging
    }, label: "Intcode Execution State")
  ...
  def amplifier_loop_sequence(intcode, phases) do
    ...
    states = Enum.map(phases, fn phase ->
      {intcode, [phase], 0} # Each amplifier's state: intcode, inputs, program counter
    end)
  ...
  defp loop(states, current_amp, input, last_output) do
    ...
    case run_intcode(intcode, [input | inputs], pc) do
      {:output, output, new_intcode, new_pc} ->
        ...
        # Check if all amplifiers have halted, returning the last output if they have
      ...
      {:await_input, new_intcode, new_pc} ->
        # Update amplifier state and pass input to the next amplifier
        new_states = List.replace_at(states, current_amp, {new_intcode, [], new_pc})
        next_amp = rem(current_amp + 1, length(states))
      ...
      {:halt, _} ->
        if current_amp == 4 do
          # Return the last output when the final amplifier halts
          last_output
        else
          next_amp = rem(current_amp + 1, length(states))
