defmodule AdventOfCode2019.Day7 do
  ...
  def run_intcode(intcode, inputs, pc \\ 0) do
    ...
    IO.inspect(%{
      pc: pc,
      opcode: opcode,
      intcode: Enum.slice(intcode, pc, 10)  # Print a portion of the intcode for debugging
  ...
  def amplifier_loop_sequence(intcode, phases) do
    ...
    states = Enum.map(phases, fn phase ->
      {intcode, [phase], 0} # Initial state for each amplifier: intcode, inputs, and program counter
    end)
  ...
  defp loop(states, current_amp, input) do
    ...
    case run_intcode(intcode, [input | inputs], pc) do
      ...
      {:halt, _} ->
        # If all amplifiers have halted, return the last output
        if current_amp == 4 do
          input
        else
          next_amp = rem(current_amp + 1, length(states))
