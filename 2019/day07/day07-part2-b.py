...
def amplifier_loop_sequence(intcode, phases):
    ...
    signal = 0  # Initial input signal
    halted = [False] * len(phases)  # Track which amplifiers are halted

    while not all(halted):  # Continue until all amplifiers halt
        for i in range(len(phases)):
            if not halted[i]:
                ...
                if result[0] == 'output':
                    signal = result[1]
                    states[i] = (result[2], [], result[3])  # Update state
                elif result[0] == 'await_input':
                    states[i] = (result[1], [], result[2])  # Store state, wait for next input
                elif result[0] == 'halt':
                    halted[i] = True  # Mark this amplifier as halted
    return signal
