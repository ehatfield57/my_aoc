...
;  example, you and Santa. You download a map of the local orbits
;  (your puzzle input).
;
...
;
; What is the total number of direct and indirect orbits in your map data? (Answer: 314702)
;

(define (get-orbits filename)
  (let ((orbits (make-hash))
        (lines (file->lines filename)))
    (for ((line lines))
         (define-values (a b) (apply values (string-split line ")")))
;         (if (hash-has-key? orbits a)
;             (hash-set! orbits a (cons b (hash-ref orbits a)))
;             (hash-set! orbits a (list b))) 
         (if (hash-has-key? orbits b)
             (error "Hey!  That's not supposed to happen!")
             (hash-set! orbits b a)))
    orbits))

(define (part-one filename)
  (let ((orbits (get-orbits filename))
        (total 0))
    (for ((planet (hash-keys orbits)))
         (let loop ((cur-planet planet))
          (cond
            ((string=? "COM" cur-planet)
             (set! total (+ 0 total)))
            (else
              (set! total (add1 total))
              (loop (hash-ref orbits cur-planet))))))
    total))

(check-equal? (part-one "test-data.txt") 42)
(check-equal? (part-one "day06-data.txt") 314702)
...
;  they are orbiting - not between YOU and SAN.)  (Answer: 439)
;
...
(define (part-two filename)
  (let ((lines (file->lines filename))
    ...
    (define orbit-graph (unweighted-graph/undirected edges))
    (define-values (dist _) (dijkstra orbit-graph "YOU"))

    (define output (open-output-file "test2.dot" #:exists 'replace))
    (graphviz orbit-graph #:output output)
    (close-output-port output)

    (- (hash-ref dist "SAN") 2)))

(check-equal? (part-two "test2-data.txt") 4)
(check-equal? (part-two "day06-data.txt") 439)
