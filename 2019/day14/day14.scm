...
;  only exception, ORE, is the raw material input to the entire process and
...
;  10 costing 10 ORE) and as much of chemical B as you want (each costing 1 ORE).
...
;  finally E into FUEL. (30 A is produced because its reaction requires that
;  it is created in increments of 10.)
;
...
;  amount of ORE required to produce exactly 1 FUEL? (Answer: 261960)
;

; Used Reddit Python Solution

(define (get-data filename)
  (let ((data (make-hash))
        (lines (file->lines filename))
        (get-pair (lambda (str)
                    (define-values (batch key) (apply values (string-split str)))
                    (list (string->number batch) key))))
    (for ((line lines))
         (let ((parts (reverse (regexp-match* #rx"([0-9]+) ([A-Z]+)" line)))
                (key #f)
                (batch 0)
                (stuff '()))
           (for ((part parts))
                (define foo (get-pair part))
                (cond
                  ((not key) (set! key (second foo))
                             (set! batch (first foo)))
                  (else
                    (set! stuff (cons foo stuff)))))
           (hash-set! data key (list batch stuff))))
    data))

(define (reduce reactions chemical amounts)
  (let* ((data (hash-ref reactions chemical))
         (divisor (first data)))
    (printf "Hi Edward A, chemical: ~a, divisor: ~a~n" chemical divisor)
    (for ((needed (second data)))
         (printf "Hi Edward B, multiplier: ~a~n" (first needed))
         (cond
           ((string=? "ORE" (second needed))
            (printf "Hi Edward C, ORE: ~a~n" needed))
           (else
             (reduce reactions (second needed) amounts))))
    ))
