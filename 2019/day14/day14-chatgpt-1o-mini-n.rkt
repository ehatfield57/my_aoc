...
(define (parse-reactions input)
  ...
  ;; Flatten the list of (key . value) pairs into a flat list: key1 val1 key2 val2 ...
  (define flat-list (apply append (map (lambda (pair) (list (car pair) (cdr pair))) reactions-list)))
  ;; Build the hash table
  (apply hash flat-list))
...
(define (compute-ore reactions fuel-amount)
  ...
  ;; Now, process any remaining ORE
  (define ore-needed (hash-ref needs 'ORE 0))
  (set! total-ore (+ total-ore ore-needed))
