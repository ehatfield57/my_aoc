...
(define (parse-reaction-line line)
  (define [inputs-str output-str] (string-split line " => "))
  (define inputs
    (map (λ (s)
...
(define (compute-ore reactions fuel-amount)
  ...
  (define (get-reaction chem)
    (hash-ref reactions chem))
  ...
  (define (produce chem amount)
    (if (eq? chem ORE)
        ...
        (begin
          ...
          (when (> amount 0)
            (define reaction (get-reaction chem))
...
;; Read input from a file or define it as a string
(define input 
  "10 ORE => 10 A
1 ORE => 1 B
7 A, 1 B => 1 C
