...
(define (compute-ore reactions fuel-amount)
  ...
  (define (loop)
    ;; Get the list of chemicals currently needed
    (define keys (hash-keys needs))
    (unless (empty? keys)
      ...
      (if (eq? chem 'ORE)
          ...
          (let* ([available (hash-ref surplus chem 0)] ; Available surplus
                 ...
                 [output-qty (reaction-output-qty reaction)]
                 [times (ceiling (/ remaining-amt output-qty))] ; Number of reaction runs
                 [produced (* times output-qty)] ; Total produced from reactions
            ...
            (for-each (lambda (input)
                        (let ([input-chem (car input)]
                          ...
                          ;; Update needs with the required input quantity
  ...
  (define (binary-search low high)
    (if (> low high)
        ...
        (let* ([mid (quotient (+ low high) 2)]
          ...
          (cond
            [(> required total-ore-available) (binary-search low (- mid 1))] ; Reduce upper
            [else (binary-search mid high)])))) ; Increase lower
