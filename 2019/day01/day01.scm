...
(define (calc-fuel mass) (inexact->exact (- (truncate (/ mass 3.0)) 2)))
...
(define (part-one filename)
  (let ((lines (file->lines filename))
        (total-fuel 0))
   (for ((line lines))
        (set! total-fuel (+ total-fuel (calc-fuel (string->number line)))))
   total-fuel))

(check-equal? (part-one "day01-data.txt") 3273715)
...
(define (include-fuel fuel)
  (if (<= fuel 0)
      0
      (+ fuel (include-fuel (calc-fuel fuel)))))

(check-equal? (include-fuel 2) 0)
(check-equal? (include-fuel 654) 966)
(check-equal? (include-fuel 33583) 50346)
(check-equal? (include-fuel 3273715) 4910529)

(define (part-two filename)
  (let ((lines (file->lines filename))
        (total-fuel 0))
   (for ((line lines))
        (let ((line-fuel (calc-fuel (string->number line))))
        (set! total-fuel (+ total-fuel (include-fuel line-fuel)))
        (printf "Mass:~a, Fuel:~a, Fuel-for-Fuel:~a, Total Fuel So far: ~a~n"
                (string->number line) line-fuel (include-fuel line-fuel) total-fuel)))
   total-fuel))

(check-equal? (part-two "day01-data.txt") 4907702)
