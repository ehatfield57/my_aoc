...
;  There's even an Intcode program (your puzzle input) that gives you access
;  to the drone system.
;
...
;
; Then, the program will output whether the drone is stationary (0) or being pulled
;  by something (1). For example, the coordinate X=0, Y=0 is directly in front of
;  the tractor beam emitter, so the drone control program will always report 1 at
;  that location.
;
...
;
; However, you'll need to scan a larger area to understand the shape of the beam. How
;  many points are affected by the tractor beam in the 50x50 area closest to the emitter?
;  (For each of X and Y, this will be 0 through 49.) (Answer: 112)
;

; (Used Reddit 2019 nim code to find solution)
...
; .............................###########
;
; In this example, the 10x10 square closest to the emitter that fits entirely within
;  the tractor beam has been marked O. Within it, the point closest to the emitter
;  (the only highlighted O) is at X=25, Y=20.
;
; Find the 100x100 square closest to the emitter that fits entirely within the tractor
;  beam; within that square, find the point closest to the emitter. What value do you
;  get if you take that point's X coordinate, multiply it by 10000, then add the point's
;  Y coordinate? (In the example above, this would be 250020.) (Answer: 18261982)
;

; (Used Reddit solution from go repository)
