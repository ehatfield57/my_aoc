...
;  (if the destination node has enough space to receive the data). The sending node
...
;
; To do this, you'd like to count the number of viable pairs of nodes. A viable pair is
...
;
; How many viable pairs of nodes are there? (Answer: ?)
;

; Eyeballed the data and got the right answer to Part One: 990
;  (One node has 88T free and 990 other nodes had < 88 to move)
...
;  you can access directly, node-x0-y0, is almost full. The node containing the data
...
;  moving any time soon). So, initially, the grid's capacities and connections look
;  like this:
;
; ( 8T/10T) --  7T/ 9T -- [ 6T/10T]
;     |           |           |
...
;
; The node you can access directly is in parentheses; the data you want starts in the
;  node marked by square brackets.
;
...
;  as _, and the very large, very full node, which we'll draw as #. Let's also draw the
...
;  #  .  .
;
; The goal is to move the data in the top right, G, to the node in parentheses. To do
;  this, we can issue some commands to the grid and rearrange the data:
;
...
;  #  .  .
;
; At this point, we're quite close. However, we have no deletion command, so we have
;  to move some more data around. So, next, we move the data from node-y1-x2 to node-y0-x2:
;
; (.) G  .
;  .  .  _
;  #  .  .
...
;
; What is the fewest number of steps required to move your goal data to node-x0-y0? (Answer: ?)
;

;
; Read file and mimic diagram:
;
; (.) .  G
;  .  _  .
;  #  .  .
...
(define (diagram filename)
  (let ((lines (file->lines filename))
        (cur-x -1))
   (for ((line lines))
        (when (regexp-match #rx"node-x" line)
          (define-values (x y size used)
                         (apply values
                                (car
                                  (regexp-match* #rx"node-x([0-9]+)-y([0-9]+) .*?([0-9]+)T .*?([0-9]+)T" line
                                    #:match-select cdr))))
          (set! x (string->number x))
          (set! y (string->number y))
          (set! size (string->number size))
          (set! used (string->number used))
;          (printf "Hi Edward x:~a, y:~a, size:~a, used:~a~n" x y size used)
          ...
          (cond
            ((and (= x 0) (= y 0))  (printf "(.)"))
            ((and (= x 30) (= y 0)) (printf " G "))
            ((= used 0)             (printf " _ "))
            ((> size 100)           (printf " # "))
            (else                   (printf " . ")))))
   (printf "~n")))

(diagram "day22-data.txt") ; '90' is not the right answer, 205 and 210 are too low.
; Not 90, 205, 206, 210, 215
...
; Ultimately used Reddit supplied html to do it manuall.  Answer: 218 steps
