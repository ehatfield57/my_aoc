...
;  elapses before the first disc is reached, and one second elapses as the capsule
;  passes from one disc to the one below it. So, if you push the button at time=100,
;  then the capsule reaches the top disc at time=101, the second disc at time=102,
...
;
; If you press the button exactly at time=0, the capsule would start to fall; it would
;  reach the first disc at time=1. Since the first disc was at position 4 at time=0, by
;  time=1 it has ticked one position forward. As a five-position disc, the next position
...
;  two positions at this point: it started at position 1, then continued to position 0, and
;  finally ended up at position 1 again. Because there's only a slot at position 0, the
;  capsule bounces away.
;
; If, however, you wait until time=5 to push the button, then when the capsule reaches each
;  disc, the first disc will have ticked forward 5+1 = 6 times (to position 0), and the
;  second disc will have ticked forward 5+2 = 7 times (also to position 0). In this case,
;  the capsule would fall through the discs and come out of the machine.
;
; However, your situation has more than two discs; you've noted their positions in your puzzle
;  input. What is the first time you can press the button to get a capsule? (Answer: 376777)
;

; ----------------------> Me Thinking <--------------------------
;
; (for ((n (range 20))) (printf "n: ~a, disc1: ~a, disc2: ~a~n" n
;                               (modulo (+ n 4 1 0) 5)
;                               (modulo (+ n 1 1 1) 2) ))
;                                          ^ ^ ^ ^  ^
;                                          | | | |  |
...
;                       disc1   disc2
(define example-data '((5 . 4) (2 . 1)))
;                       ^   ^   ^   ^
;                       |   |   |   |
;                       |   +---|---+--- Time 0 positions (zero based)
;                       +-------+----- Disc has this many positions
...
(define (disc-maker idx pos cur)
  (lambda (n) (modulo (+ n cur 1 idx) pos)))

(define (press-button data)
  (let ((discs '()) (idx 0))
   (for ((item data))
        (set! discs (cons (disc-maker idx (car item) (cdr item)) discs))
        (set! idx (add1 idx)))
   (let loop ((time 0))
    (define total (foldl (lambda (proc acc) (+ acc (proc time))) 0 discs))
;    (printf "Time: ~a, total: ~a~n" time total)
    (if (zero? total) time (loop (add1 time))))))

(check-equal? (press-button example-data) 5)

(check-equal? (press-button part-one-data) 376777)
...
;  capsule? (Answer: 3903937)
;

(define part-two-data (append part-one-data '((11 . 0))))

(check-equal? (press-button part-two-data) 3903937)
