...
;  width; you take note of the safe tiles (.) and traps (^) in the first
;  row (your puzzle input).
;
...
;  starting row), how many safe tiles are there? (Answer: 1913)
;
...
(define (next-line prev)
  (let ((new-line "")
        (padded (string-append "." prev ".")))
    (do ((idx 0 (add1 idx)))
        ((< (sub1 (string-length prev)) idx) new-line)
        (let ((test-str (substring padded idx (+ idx 3))))
         (set! new-line (string-append new-line (if (member test-str traps) "^" ".")))))))

(define (total-safe str)
  (apply + (map (lambda (c) (if (char=? c #\.) 1 0)) (string->list str))))
...
(define (count-safe tiles rows)
  (cond
    ...
    (else
;      (printf "~a~n" tiles)
      (+ (total-safe tiles) (count-safe (next-line tiles) (sub1 rows))))))
...
; --- Part Two ---
;
; How many safe tiles are there in a total of 400000 rows? (Answer: 19993564)
;
