...
;  as you don't have enough data yet to fill the disk, repeat the following steps:
;
...
; * Next, we start calculating the checksum; after one round, we have 0111110101, which
;    10 characters long (even), so we continue.
; * After two rounds, we have 01100, which is 5 characters long (odd), so we are done.
;
; In this example, the correct checksum would therefore be 01100.
;
; The first disk you have to fill has length 272. Using the initial state in your puzzle
;  input, what is the correct checksum? (Answer: "11100110111101110")
;
; Your puzzle input is 10111100110001111.
;

(define (swap01s step0)
  (let* ((step1 (string-replace step0 "1" "t"))
         (step2 (string-replace step1 "0" "1"))
         (step3 (string-replace step2 "t" "0")))
    step3))

(define (dragon seed)
   (let ((reverse (lambda (s) (list->string (reverse (string->list s))))))
    (string-append seed "0" (reverse (swap01s seed)))))

(define (checksum s)
  (if (odd? (string-length s)) s
      (let ((pairs (regexp-match* #rx"(..)" s #:match-select car)))
        (checksum (string-join (map (lambda (pair) (case pair
                                           (("11" "00") "1")
                                           (("01" "10") "0"))) pairs) "")))))

(define (part-one input-str size)
  (let loop ((chunk input-str))
   (if (>= (string-length chunk) size)
       (checksum (substring chunk 0 size))
       (loop (dragon chunk)))))

(check-equal? (part-one "10000" 20) "01100")
(check-equal? (part-one "10111100110001111" 272) "11100110111101110")

;
; --- Part Two ---
;
; The second disk you have to fill has length 35651584. Again using the initial state
;  in your puzzle input, what is the correct checksum for this disk? (Answer: "10001101010000101")
;

;(part-one "10111100110001111" 35651584) ; <-- Part Two (Takes minutes to run)
