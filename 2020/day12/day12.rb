...
def part1(filename)
  ...
  File.readlines(filename).each do |line|
    line =~ /^([NSEWLRF])(\d+)/
    cmd, num = $1, $2.to_i
#    puts "East:#{east_x}, North:#{north_y}, Facing:#{facing}, cmd:#{cmd}, num:#{num}"
    case cmd
    when "N"
      north_y += num
    when "S"
    ...
    when "W"
      east_x -= num
    when "L","R"
      facing = ((facing - num) % 360).abs if cmd == "L"
      facing = ((facing + num) % 360).abs if cmd == "R"
    when "F"
      north_y += num if facing == 0
      east_x  += num if facing == 90
      north_y -= num if facing == 180
      east_x  -= num if facing == 270
    else
      puts "Something wrong with '#{cmd}', #{num}"
    end
  end
  return east_x.abs + north_y.abs
end

class Day12Test1 < Test::Unit::TestCase
  def test_part1
    assert_equal 25,   part1("test-input.txt")
    assert_equal 1186, part1("input.txt")
  end
end
...
Ship = Struct.new(:x, :y, :facing)
Waypoint = Struct.new(:x, :y)

def part2(filename)
  ship = Ship.new(0, 0, 90)
  waypoint = Waypoint.new(10, 1)

  File.readlines(filename).each do |line|
    line =~ /^([NSEWLRF])(\d+)/
    cmd, num = $1, $2.to_i
    puts "Command: '#{line.chomp}', Ship: #{ship}, Waypoint: #{waypoint}"
    case cmd
    when "N","S","E","W"
      waypoint.y += num if cmd == "N"
      waypoint.y -= num if cmd == "S"
      waypoint.x += num if cmd == "E"
      waypoint.x -= num if cmd == "W"
    when "L","R"
      # TODO: ejh
    when "F"
      ship.x = ship.x + waypoint.x * num
      ship.y = ship.y + waypoint.y * num
    else
      puts "Something wrong with '#{cmd}', #{num}"
    end
  end
