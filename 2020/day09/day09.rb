...
def part1(filename, circle_size)
  circle = Hash.new
  index = 0
  File.foreach(filename) do |line|
    the_number = line.to_i
    if index >= circle_size
      combos = circle.values.combination(2)
      solutions = combos.select do |pair|
        pair[0] + pair[1] == the_number
      end
      puts "Problem: #{the_number} and #{circle.values}" if solutions.empty?
    end
    circle[index % circle_size] = the_number
    index += 1
  end
end

part1("test-input.txt", 5) # 127
part1("input.txt", 25)     # 36845998
...
def part2(filename, invalid_number)
  index_low = 0
  index_high = 1
  numbers = File.foreach(filename).map {|line| line.to_i}
  while index_low < index_high
    running_tot = numbers[index_low..index_high].inject(:+)
    if running_tot < invalid_number
      index_high += 1
    elsif running_tot > invalid_number
      index_low += 1
    else
      the_range = numbers[index_low..index_high]
      puts "Found number range: #{index_low}..#{index_high}, #{the_range}"
      puts " Solution: #{the_range.min + the_range.max}"
      index_low = index_high
    end
  end
end

puts part2("test-input.txt", 127) # 62
puts part2("input.txt", 36845998) # 4830226
