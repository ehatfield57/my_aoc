...
# a massive tropical storm. Before you can figure out whether it will impact
...
# differences multiplied by the number of 3-jolt differences? (Answer: 2368)
#

def part1(filename)
  distances = { 1 => 0, 2 => 0, 3 => 0}

  numbers = File.readlines(filename).map(&:to_i).sort
  numbers = numbers + [numbers[-1] + 3]

  prev = 0
  numbers.each do |n|
    diff_amt = n - prev
    distances[diff_amt] += 1
    prev = n
  end
...
class Day10Test1 < Test::Unit::TestCase
  def test_part1
    assert_equal 35,  part1("test-input.txt")
    assert_equal 220, part1("test-input2.txt")
    assert_equal 2368,  part1("input.txt")
  end
end
...
# connect the charging outlet to your device? (Answer: ?)
#

def my_part2_forever_failure(filename)
  $adapters = Hash.new
  $solutions = 0

  File.readlines(filename).map(&:to_i).each {|n| $adapters[n] = 0}
  $max_num = $adapters.keys.max + 3
  $adapters[$max_num] = 0

  def helper(start, numbers)
    if $max_num == start
      $solutions += 1
    elsif start > $max_num
      # Noop
      ...
      [1, 2, 3].each do |jolt|
        helper(start + jolt, numbers + [start + jolt]) if $adapters.key?(start + jolt)
      end
    end
  end
...
# Thank you ChatGPT 4o for converting python code to ruby
def count_arrangements(differences)
  differences.chunk_while { |i, j| i == 1 && j == i }
             .select { |group| group.first == 1 && group.length > 1 }
             .map { |group| (2 ** (group.length - 1)) - (group.length == 4 ? 1 : 0) }
             .inject(1, :*)
end

# Thank you Reddit Python solution
def part2(filename)
  numbers = File.readlines(filename).map(&:to_i).sort
  max_num = numbers[-1] + 3
  differences = ([0] + numbers + [max_num]).each_cons(2).map {|a, b| b - a}
  solution = count_arrangements(differences)
end

class Day10Test2 < Test::Unit::TestCase
  def test_part2
    assert_equal 8,             part2("test-input.txt")
    assert_equal 19208,         part2("test-input2.txt")
    assert_equal 1727094849536, part2("input.txt")
  end
end
