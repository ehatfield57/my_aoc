...
# cid as optional. In your batch file, how many passports are valid? (Answer: 210)
#
...
def get_data(filename)
  ...
  lines.each {|line|
    if line.chomp == ""
      ...
      line.scan(/(\S+?):(\S+)/).each {|key, val| record[key] = val}
    end
  }
...
def part1(filename)
  ...
  records.each do
    |record|
    count = count + 1 if required.all?{|key|
      record.has_key? key}
  end
  count
end

puts "Part 1, test data: #{part1("test-input.txt")}" #   2
puts "Part 1, true data: #{part1("input.txt")}"      # 210
...
# how many passports are valid? (answer: 131)
#
...
def record_ok?(record)
  ...
  record.each do
    ...
    case key
    when "byr"
      ok = false if (not (value =~ /^\d\d\d\d$/)) or (value.to_i < 1920) or (value.to_i > 2002)
    when "iyr"
      ok = false if (not (value =~ /^\d\d\d\d$/)) or (value.to_i < 2010) or (value.to_i > 2020)
    when "eyr"
      ok = false if (not (value =~ /^\d\d\d\d$/)) or (value.to_i < 2020) or (value.to_i > 2030)
    when "hgt"
      if value =~ /^(\d+)cm$/
        ok = false if $1.to_i < 150 or $1.to_i > 193
      elsif value =~ /^(\d+)in$/
        ok = false if $1.to_i < 59 or $1.to_i > 76
      else
        ok = false
      end
    when "hcl"
      ok = false if (not (value =~ /^\#[0-9a-f]{6}$/))
    when "ecl"
      ok = false if (not (["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].include? value))
    when "pid"
      ok = false if (not (value =~ /^\d{9}$/))
    when "cid"
      # -- Whatever
    else
      puts "Unknown key '#{key}' found in record: #{record}"
    end
  end
  ok
end
...
def part2(filename)
  ...
  records.each do
    |record|
    count = count + 1 if (record_ok? record) and required.all?{|key| record.has_key? key}
  end
  count
end
...
class TestPartOne < Test::Unit::TestCase
  def test_byr
    assert_equal(true,  record_ok?({ "byr" => "2002" }))
    assert_equal(false, record_ok?({ "byr" => "2003" }))
  end

  def test_hgt
    assert_equal(true,  record_ok?({ "hgt" => "60in" }))
    assert_equal(true,  record_ok?({ "hgt" => "190cm" }))
    assert_equal(false, record_ok?({ "hgt" => "190in" }))
    assert_equal(false, record_ok?({ "hgt" => "190" }))
  end

  def test_hcl
    assert_equal(true,  record_ok?({ "hcl" => "#123abc" }))
    assert_equal(false, record_ok?({ "hcl" => "#123abz" }))
    assert_equal(false, record_ok?({ "hcl" => "123abc"  }))
  end

  def test_ecl
    assert_equal(true,  record_ok?({ "ecl" => "brn" }))
    assert_equal(false, record_ok?({ "ecl" => "wat" }))
  end

  def test_pid
    assert_equal(true,  record_ok?({ "pid" => "000000001" }))
    assert_equal(false, record_ok?({ "pid" => "0123456789" }))
  end

  def test_data
    assert_equal(2, part2("test-input.txt"))
    assert_equal(0, part2("test1-input.txt"))
    assert_equal(4, part2("test2-input.txt"))
  end
end

puts "Part 2 solution: #{part2("input.txt")}"
