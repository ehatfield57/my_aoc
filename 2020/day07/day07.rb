...
# list of rules is quite long; make sure you get all of it.) (Answer: 169)
#
...
def get_data(filename)
  ...
  lines.each do |line|
    ...
    if line =~ /^(.*?) bags? contain(.*)/
      bag_key, the_rest = $1, $2
      data[bag_key] = []
      if the_rest != " no other bags." then
        the_rest.scan(/ (\d+) (.*?) bags?[.,]/).each do |num, bag|
          data[bag_key] << [num.to_i, bag]
        end
      end
    end
  end
...
def reverse(data)
  reversed = Hash.new()
  data.keys.each do |bag_key|
    data[bag_key].each do |num_bag|
      if reversed.has_key?(num_bag[1])
        reversed[num_bag[1]] << bag_key
      else
        reversed[num_bag[1]] = [bag_key]
      end
    end
...
def part1(filename)
  ...
  while !queue.empty?
    bag = queue.shift
    unique[bag] = true
    rev_data[bag].each {|parent| queue << parent} if rev_data.has_key?(bag)
  end
  unique.keys.count - 1
end
...
class TestPartOne < Test::Unit::TestCase
  def test_part1
    assert_equal(4, part1("test-input.txt"))
    assert_equal(169, part1("input.txt"))
  end
end
...
def helper(data, bag)
  ...
  if content.length == 0
    ...
    data[bag].inject(1) do |sum, num_bag|
      sum += (num_bag[0] * helper(data, num_bag[1]))
    end
  end
end
...
def part2(filename)
  unique = Hash.new()
  data = get_data(filename)
  return helper(data, "shiny gold") - 1 # Don't count shiny gold bag
end

class TestPartTwo < Test::Unit::TestCase
  def test_part2
    assert_equal(32, part2("test-input.txt"))
    assert_equal(126, part2("test-input2.txt"))
    assert_equal(82372, part2("input.txt"))
  end
end
