...
def get_data(filename)
  numbers = []
  File.open(filename).each { |line| numbers << line.to_i }
  numbers
end

def part1(filename)
  data = get_data(filename)
  data.each_with_index do | number1, i|
    data[(i + 1)..-1].each do | number2|
...
def part2(filename)
  data = get_data(filename)
  data.each_with_index do | number1, i|
    data[(i + 1)..-2].each do | number2|
      data[(i + 2)..-1].each do | number3|
...
class TestPartOne < Test::Unit::TestCase
  def test_part1
    assert_equal(514579, part1("test-input.txt"))
    assert_equal(381699, part1("input.txt"))
  end

  def test_part2
    assert_equal(241861950, part2("test-input.txt"))
    assert_equal(111605670, part2("input.txt"))
  end
end
